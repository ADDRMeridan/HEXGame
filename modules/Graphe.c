...
void ajout_dans_groupe(Hexa hex_1, Hexa hex_2){
	...
	if( hex_1 != NULL && hex_2 != NULL && hex_adjacent(hex_1, hex_2) && couleur_sommet(hex_1) != TRANSPARANT && couleur_sommet(hex_1) == couleur_sommet(hex_2) ){
		...
		/* Si hex_1 et hex_2 sont deja dans un groupe */
			...
			for( int i=groupe -> nb_aretes-1; i<( groupe -> nb_aretes + groupe_2 -> nb_aretes ); i++ ){
 ...
 *
 */
Intersection intersection(Hexa u, Hexa v){
	Intersection intersec;
	intersec . tab_intersection = (Hexa*) malloc( sizeof( Hexa ) );
	assert( intersec . tab_intersection != NULL );
	intersec . cardinal = 0;

	for(int i=0; i<6; i++){
		for(int e=0; e<6; e++){
			if( u -> aretes[i] == v -> aretes[e] ){
				intersec . tab_intersection = (Hexa*) realloc(intersec . tab_intersection, ( intersec . cardinal + 1 ) * sizeof(Hexa) );
				assert( intersec . tab_intersection != NULL );

				intersec . tab_intersection[intersec . cardinal] = u -> aretes[i];
				(intersec . cardinal)++;
			}
		}
	}
...
bool connexion_forcer(Hexa u, Hexa v){
	Intersection i = intersection( u, v );

	if( couleur_sommet( u ) ==  couleur_sommet( v ) && distance_hexagones( u, v ) == 3
	&& i . cardinal >= 2 && couleur_sommet(i . tab_intersection[0]) == TRANSPARANT 
	&& couleur_sommet(i . tab_intersection[1]) == TRANSPARANT ){
		return true;
	}
